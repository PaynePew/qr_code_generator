...
def aggregate_scans(scans: list[Scan], *, recent_limit: int = DEFAULT_RECENT_LIMIT) -> dict:
    return {
...
def _scans_by_day(scans: list[Scan]) -> list[dict]:
    day_data: dict[str, dict] = defaultdict(lambda: {"count": 0, "status_codes": defaultdict(int)})
    for scan in scans:
